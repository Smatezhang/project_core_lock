                ...
                .writeCharacteristic(
                        ...
                        new BleWriteCallback() {
                            ...
                            public void onWriteSuccess(String mac, int current, int total, byte[] justWrite) {
                                ...
                                if (mCallback != null) {
                                    mCallback.onWriteSuccess(mBleBluetooth.getDevice().getMac(),position, mTotalNum, justWrite);
                                }
                            ...
                            public void onWriteFailure(String mac, BleException exception) {
                                if (mCallback != null) {
                                    mCallback.onWriteFailure(mBleBluetooth.getDevice().getMac(),new OtherException("exception occur while writing: " + exception.getDescription()));
                                }
                                if (mSendNextWhenLastSuccess) {
                                    Message message = mHandler.obtainMessage(BleMsg.MSG_SPLIT_WRITE_NEXT);
                                    mHandler.sendMessageDelayed(message, mIntervalBetweenTwoPackage);
