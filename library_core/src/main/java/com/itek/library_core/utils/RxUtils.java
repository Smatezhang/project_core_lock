...
import com.itek.library_core.util.ExceptionHandle;
    ...
    public static ObservableTransformer exceptionTransformer() {
        return new ObservableTransformer() {
            ...
            public ObservableSource apply(Observable observable) {
                return observable
//                        .mipmap-xxhdpi(new HandleFuc<T>())  //这里可以取出BaseResponse中的Result
    ...
    private static class HandleFuc<T> implements Function<BaseResponse<T>, T> {
        ...
        public T apply(BaseResponse<T> response) {
            if (!response.isOk())
                throw new RuntimeException(!"".equals( response.getMessage()) ? response.getMessage() : "");
